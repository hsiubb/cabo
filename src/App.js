...
function App() {
  ...
  function mockOpponentTurn(self = 2) {
    ...
    if(knowAllSelfCard) {
      console.log('knowAllSelfCard');
      console.log(`player ${self} status: knowing all card it owns.`);

      let playerList = [playerOneCards, playerTwoCards, playerThreeCards, playerFourCards].slice(0, gamePlayer);
      console.log('[playerOneCards, playerTwoCards, playerThreeCards, playerFourCards].slice(0, gamePlayer)');
      console.log(playerList);
      console.log(playerList.length);
      let resMin = Math.min(...playerList.map(cards => {
      // let resMin = Math.min([playerOneCards, playerTwoCards].map(cards => {
    ...
    if((unknowCard !== false) && (discardPile[0] < LOGICALLY_SMALL)) {
      console.log('discardPile[0] < LOGICALLY_SMALL');
      console.log('unknowCard:', unknowCard);
      console.log('ownCards[unknowCard]:', ownCards[unknowCard]);
      console.log('ownCards[unknowCard]?.number:', ownCards[unknowCard]?.number);
        ...
        case 12:
          ...
          playerOneCards.map((card, i) => {
            if(card.comView) {
              ...
              if((ownMin == 8) || (card.number < playerOneCards[ownMin].number)) {
                ownMin = i;
              }
            }
          });
          ...
          if(hasKnow) {
            ...
            pushLog(`opponent check min [${ownMin}]: ${ownMin}`);
            pushLog(`opponent check min [${ownMin}]: ${playerOneCards}`);
            pushLog(`opponent check min [${ownMin}]: ${playerOneCards[ownMin]}`);
        ...
        gamePlayer ? 
          ...
          <div className="playing-board">
            ...
            <div onClick={() => viewRule(true)}>
              ...
              <span className="var btn">rule</span>
